...
const PlannerProgress = ({ refresh }) => {
  ...
  if (loading) return <p className="text-center text-gray-500">Loading progress...</p>;
  if (!progressData) return <p className="text-center text-gray-500">No progress data available.</p>;

  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      {Object.entries(progressData).map(([category, { limit, spent }]) => {
        const percent = limit ? Math.min((spent / limit) * 100, 100) : 0;
        const progressColor = percent < 70 ? "bg-teal-500" : percent < 100 ? "bg-yellow-400" : "bg-red-500";

        return (
          <div key={category} className="flex items-center gap-4 bg-white p-3 rounded-lg shadow-md">
            <div className="w-48 text-gray-700 font-semibold truncate">{category}</div>
